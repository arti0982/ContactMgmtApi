...
public class CustomerService {
    ...
    public List<String> getPhoneNumbersByCustomer(String customerId) {
        ...
        return StaticContactsData.CONTACT_MAP.get(customerId).getPhoneNumbers().stream().map(PhoneNumber::getNumber).toList();
    }
    ...
    public void activateCustomerPhoneNumber(String customerId, String customerPhoneNumber) {
        if ( StaticContactsData.CONTACT_MAP.get(String.valueOf(customerId)) == null)
            throw new ResourceNotFoundException("Could not find the customer for customer id '" + customerId + "'");

        PhoneNumber phoneNumberToActivate = StaticContactsData.CONTACT_MAP.get(customerId).getPhoneNumbers().stream()
        ...
        phoneNumberToActivate.setActive("Y");
    }
}
